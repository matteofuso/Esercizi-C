...
int main(int argc, char *argv[])
{
    ...
    read(socketfd, &sommamediaPari, sizeof(struct SommaMedia));
    read(socketfd, &sommamediaDispari, sizeof(struct SommaMedia));
    printf("La somma dei numeri pari è %d, la media %.2f\n", sommamediaPari.somma, sommamediaPari.media);
    printf("La somma dei numeri dispari è %d, la media %.2f\n", sommamediaDispari.somma, sommamediaDispari.media);
    close(socketfd);
    return 0;
}